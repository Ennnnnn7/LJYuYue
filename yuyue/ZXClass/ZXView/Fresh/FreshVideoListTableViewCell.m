...
@interface FreshVideoListTableViewCell() <UserInfoDelegate>
//判断是否下载的对话框
...
@implementation FreshVideoListTableViewCell

- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    
    //判断网络
    [[JudgeMentInternet defaultInstance] changeInternet];
...
- (void)collect:(UIButton *)button {
    ...
    if (![UserInfo sharedUserInfo].isLogin) {
        ...
        [self collected];
        if ( _fresh.isCollect) {
NSLog(@"FreshVideoListTableViewCell-----进入下载提示框");
...
NSLog(@"FreshVideoListTableViewCell-----从沙盒中删除 , 还没有写 , 视频名字%@",self.fresh.videoName);
                [self removeFile];
            ...
            //判断是否正在下载 , 如果是,就取消下载
            if (self.fresh.isDownload) {
                [self cancelDownload];
                self.downBlock(@"NO");
            }
NSLog(@"FreshVideoListTableViewCell--------弹出取消关注的窗口");
            self.reloadDataBlock();
        }
...
- (void)play:(UIButton *)button {
    self.movieBlock(_fresh.videouri);
}
...
NSLog(@"FreshVideoListTableViewCell-----下载的网络状态%@" , judgeMent.status);
        ...
        if(_internet == nil){
            ...
            if (buttonIndex == 1) {
                ...
                if (judgeMent.isWifi) {
NSLog(@"FreshVideoListTableViewCell-----开始下载");
                    //接收返回视频的名字
                    [DownLoadVideo defaultInstance].pushName = ^(NSString *str){
                        self.fresh.videoName = str;
NSLog(@"FreshVideoListTableViewCell-----------\\\%@",str);
                    };
                    self.fresh.isDownload = YES;//确认下载
                    self.downBlock(_fresh.videouri);
...
NSLog(@"FreshVideoListTableViewCell-----第一个对话框取消");
                self.fresh.isDownload = NO;//不下载
            }
...
NSLog(@"FreshVideoListTableViewCell-----不管点那个都没用");
                self.fresh.isDownload = NO;
                
            } else{
                
                if (buttonIndex == 1) {
NSLog(@"FreshVideoListTableViewCell-----直接下");
                    //接收返回视频的名字
                    [DownLoadVideo defaultInstance].pushName = ^(NSString *str){
                        self.fresh.videoName = str;
NSLog(@"FreshVideoListTableViewCell--------\\\%@",str);
...
NSLog(@"FreshVideoListTableViewCell-----不下载");
                    self.fresh.isCollect = NO;
                }
